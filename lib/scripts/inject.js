...
function totalMessages() {
	return [].slice.call(document.querySelectorAll('.previewline')).length;
}
...
function noPhotoMessages() {
	return [].slice.call(document.querySelectorAll('.photo')).filter(function (photo) {
		return photo.children[0].src === 'https://k0.okccdn.com/media/img/user/placeholder_2013/pq_60.png';
	});
}

function anyMessages() {
	return _.some([noPhotoMessages().length, weakMessageCount(), countsForPhrase("it's a match")], function (item) {
		return item > 0;
	});
}
...
function deleteMessagesByPhrases(phrases) {
	phrases.forEach(function (phrase) {
		[].slice.call(document.querySelectorAll('.previewline')).filter(function (line) {
			return line.innerText.toLowerCase().includes(phrase.toLowerCase());
		}).map(function (el) {
...
function deleteOldMessages() {
	...
	messages.filter(function (message) {
		id = document.querySelector('#' + message.id + ' .fancydate').id;
		date = new Date(id.split('_')[1] / 1000);
		return (new Date() - date) / 60 / 60 / 24 > 90;
	}).length.map(function (message) {
...
function deleteNoPhotos() {
	window.delay = 0;
	[].slice.call(document.querySelectorAll('.photo')).filter(function (photo) {
		return photo.children[0].src === 'https://k0.okccdn.com/media/img/user/placeholder_2013/pq_60.png';
	}).map(function (el, idx) {
		id = el.parentElement.parentElement.parentElement.id;
		setTimeout(function () {
...
function nextMatch() {
	if (location.pathname === '/messages' && !location.search.includes('readmsg')) {
		...
		messages.filter(function (message) {
			...
			document.querySelector('#' + message.id + ' .open').click();
			throw BreakException;
		});
	}

	if (document.querySelector('.desc').innerText.includes('perfect time')) {
